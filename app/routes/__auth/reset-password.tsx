...
import { commitAuthSession, getAuthSession, refreshAccessToken, updateAccountPassword } from '~/modules/auth';
...
export async function action({ request }: ActionArgs) {
  ...
  const result = await ResetPasswordSchema.safeParseAsync(parseFormAny(formData));
    ...
    <div className="flex flex-col justify-center min-h-full">
      <div className="w-full max-w-md px-8 mx-auto">
        <Form ref={zo.ref} method="post" className="space-y-6" replace>
          ...
          <div>
            <label htmlFor={zo.fields.confirmPassword()} className="block text-sm font-medium text-gray-700">
          ...
          <input type="hidden" name={zo.fields.refreshToken()} value={userRefreshToken} />
          <button
        ...
        {actionData?.message ? (
          <div className="flex flex-col items-center">
            <div className={tw(`mb-2 h-6 text-center text-red-600`)}>{actionData.message}</div>
